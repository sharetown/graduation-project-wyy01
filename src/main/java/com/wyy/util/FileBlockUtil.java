...
public class FileBlockUtil {
	...
	public void initTempDir() {
		...
		if (f.isDirectory()) {
			try {
				...
				lu.writeException(e);
				System.out.println("错误：临时文件清理失败，请手动清理" + f.getAbsolutePath() + "文件夹内的临时文件。");
			}
		} else {
			if (!f.mkdir()) {
				System.out.println("错误：无法创建临时文件夹" + f.getAbsolutePath() + "，请检查主文件系统存储路径是否可用。");
			}
		}
	}
	...
	public File saveToFileBlocks(final MultipartFile f) {
		...
		try {
			...
			lu.writeException(e);
			System.out.println("错误：文件块生成失败，无法存入新的文件数据。详细信息：" + e.getMessage());
		}
		return null;
	}
	...
	public File getFileFromBlocks(Node f) {
		...
		try {
			...
			lu.writeException(e);
			System.out.println("错误：文件数据读取失败。详细信息：" + e.getMessage());
		}
		return null;
	}
		...
		Thread checkThread = new Thread(() -> {
			...
			for (File path : paths) {
				try (DirectoryStream<Path> ds = Files.newDirectoryStream(path.toPath())) {
					...
					System.out.println("警告：文件节点效验时发生意外错误，可能未能正确完成文件节点效验。错误信息：" + e.getMessage());
					lu.writeException(e);
				}
			}
		});
