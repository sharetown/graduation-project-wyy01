...
import com.wyy.enumer.LogLevel;
import com.wyy.enumer.VCLevel;
import com.wyy.pojo.ExtendStores;
import com.wyy.pojo.Folder;
import com.wyy.pojo.ServerSetting;
...
public class ConfigureReader {
	...
	private ConfigureReader() {
		...
		if (!serverProp.isFile()) {
			System.out.println("服务器配置文件不存在，需要初始化服务器配置。");
		...
		if (!accountProp.isFile()) {
			System.out.println("用户账户配置文件不存在，需要初始化账户配置。");
			this.createDefaultAccountPropertiesFile();
		}
		try {
			System.out.println("正在载入配置文件...");
			...
			initIPRules();
			initSignUpRules();
			System.out.println("配置文件载入完毕。正在检查配置...");
			...
			System.out.println("错误：无法加载一个或多个配置文件（位于" + this.confdir + "路径下），请尝试删除旧的配置文件并重新启动本应用或查看安装路径的权限（必须可读写）。");
		}
	}
		...
		if (ss != null) {
			...
			if (this.testServerPropertiesAndEffect() == 0) {
				try {
					...
					System.out.println("配置更新完毕，准备就绪。");
					return true;
				} catch (Exception e) {
					System.out.println("错误：更新设置失败，无法存入设置文件。");
				}
			}
		}
		return false;
	...
	private int testServerPropertiesAndEffect() {
		System.out.println("正在检查服务器配置...");
		final String pMustLogin = this.serverp.getProperty("mustLogin");
		if (pMustLogin == null) {
			System.out.println("警告：未找到是否必须登录配置，将采用默认值（O）。");
			this.mustLogin = "O";
		} else {
			if (!"N".equals(pMustLogin) && !"O".equals(pMustLogin)) {
				System.out.println("错误：必须登入功能配置不正确（只能设置为“O”或“N”），请重新检查。");
		...
		if (ports == null) {
			System.out.println("警告：未找到端口配置，将采用默认值（8080）。");
			this.port = 8080;
		} else {
			try {
				this.port = Integer.parseInt(ports);
				if (this.port <= 0 || this.port > 65535) {
					System.out.println("错误：端口号配置不正确，必须使用1-65535之间的整数。");
					return INVALID_PORT;
				}
			} catch (Exception e) {
				System.out.println("错误：端口号配置不正确，必须使用1-65535之间的整数。");
		...
		if (logs == null) {
			System.out.println("警告：未找到日志等级配置，将采用默认值（E）。");
			this.log = "E";
		} else {
			if (!logs.equals("N") && !logs.equals("R") && !logs.equals("E")) {
				System.out.println("错误：日志等级配置不正确（只能设置为“N”、“R”或“E”），请重新检查。");
				return INVALID_LOG;
			}
			this.log = logs;
		}
		...
		if (vcl == null) {
			System.out.println("警告：未找到登录验证码配置，将采用默认值（STANDARD）。");
			...
			case "CLOSE":
				...
				break;
			default:
				System.out.println("错误：登录验证码配置不正确（只能设置为“STANDARD”、“SIMP”或“CLOSE”），请重新检查。");
				return INVALID_VC;
			}
		}
		// 是否允许用户修改密码
		...
		if (changePassword == null) {
			System.out.println("警告：未找到用户修改密码功能配置，将采用默认值（禁用）。");
			...
			case "N":
				...
				break;
			default:
				System.out.println("错误：用户修改账户密码功能配置不正确（只能设置为“Y”或“N”），请重新检查。");
		...
		if (fileChain == null) {
			System.out.println("警告：未找到永久资源链接功能配置，将采用默认值（禁用）。");
			...
			case "CLOSE":
				...
				break;
			default:
				System.out.println("错误：永久资源链接功能配置不正确（只能设置为“OPEN”或“CLOSE”），请重新检查。");
		...
		if (bufferSizes == null) {
			System.out.println("警告：未找到缓冲大小配置，将采用默认值（1048576）。");
			this.bufferSize = 1048576;
		} else {
			try {
				this.bufferSize = Integer.parseInt(bufferSizes);
				if (this.bufferSize <= 0) {
					System.out.println("错误：缓冲区大小设置无效。");
		...
		if (this.FSPath == null) {
			System.out.println("警告：未找到主文件系统路径配置，将采用默认值。");
		...
		if (!fsFile.isDirectory() || !fsFile.canRead() || !fsFile.canWrite()) {
			System.out.println("错误：文件系统路径[" + this.fileSystemPath + "]无效，该路径必须指向一个具备读写权限的文件夹。");
		...
		for (ExtendStores es : extendStores) {
			if (!es.getPath().isDirectory() || !es.getPath().canRead() || !es.getPath().canWrite()) {
				System.out.println("错误：扩展存储区路径[" + es.getPath().getAbsolutePath() + "]无效，该路径必须指向一个具备读写权限的文件夹。");
				return INVALID_FILE_SYSTEM_PATH;
			}
		}
		...
		if (!fbFile.isDirectory() && !fbFile.mkdirs()) {
			System.out.println("错误：无法创建文件块存放区[" + this.fileBlockPath + "]。");
		...
		if (!fnFile.isDirectory() && !fnFile.mkdirs()) {
			System.out.println("错误：无法创建文件节点存放区[" + this.fileNodePath + "]。");
		...
		if (!tfFile.isDirectory() && !tfFile.mkdirs()) {
			System.out.println("错误：无法创建临时文件存放区[" + this.TFPath + "]。");
		...
		if ("true".equals(serverp.getProperty("mysql.enable"))) {
			...
			if (url.indexOf("/") <= 0 || url.substring(url.indexOf("/")).length() == 1) {
				System.out.println("错误：自定义数据库的URL中必须指定数据库名称。");
		...
		if (enableHttps != null) {
			if ("true".equals(enableHttps)) {
				...
				if (httpsports == null) {
					System.out.println("警告：未找到https端口配置，将采用默认值（443）。");
					httpsPort = 443;
				} else {
					try {
						this.httpsPort = Integer.parseInt(httpsports);
						if (httpsPort <= 0 || httpsPort > 65535) {
							System.out.println("错误：无法启用https支持，https访问端口号配置不正确。");
							return HTTPS_SETTING_ERROR;
						}
					} catch (Exception e) {
						System.out.println("错误：无法启用https支持，https访问端口号配置不正确。");
				...
				System.out.println("错误：https支持功能的启用项配置不正确（只能设置为“true”或“false”），请重新检查。");
				return HTTPS_SETTING_ERROR;
			}
		}
		// 是否启用XFF解析
		...
		if (xffConf != null) {
			...
			case "enable":
				...
				break;
			default:
				System.out.println("错误：IP地址xff解析配置不正确（只能设置为“disable”或“enable”），请重新检查。");
		...
		if (ffmpegConf != null) {
			...
			case "enable":
				...
				break;
			default:
				System.out.println("错误：视频播放功能的在线解码配置不正确（只能设置为“disable”或“enable”），请重新检查。");
		...
		if (downloadZipConf != null) {
			...
			case "enable":
				...
				break;
			default:
				System.out.println("错误：“打包下载”功能的配置不正确（只能设置为“disable”或“enable”），请重新检查。");
	...
	public void createDefaultServerPropertiesFile() {
		System.out.println("正在生成初始服务器配置文件（" + this.confdir + SERVER_PROPERTIES_FILE + "）...");
		...
		try {
			...
			System.out.println("初始服务器配置文件生成完毕。");
		} catch (FileNotFoundException e) {
			System.out.println("错误：无法生成初始服务器配置文件，存储路径不存在。");
		} catch (IOException e2) {
			System.out.println("错误：无法生成初始服务器配置文件，写入失败。");
		}
	}

	private void createDefaultAccountPropertiesFile() {
		System.out.println("正在生成初始账户配置文件（" + this.confdir + ACCOUNT_PROPERTIES_FILE + "）...");
		...
		try (FileOutputStream accountSettingOut = new FileOutputStream(this.confdir + ACCOUNT_PROPERTIES_FILE)) {
			...
			System.out.println("初始账户配置文件生成完毕。");
		} catch (FileNotFoundException e) {
			System.out.println("错误：无法生成初始账户配置文件，存储路径不存在。");
		} catch (IOException e2) {
			System.out.println("错误：无法生成初始账户配置文件，写入失败。");
		}
	}
						...
						for (WatchEvent<?> we : es) {
							if (ACCOUNT_PROPERTIES_FILE.equals(we.context().toString())) {
								...
								if (accountProp.isFile() && accountProp.canRead()) {
									...
									initIPRules();
									initSignUpRules();
									System.out.println("账户配置更新完成，已加载最新配置。");
								} else {
									accountp.clear();
									System.out.println("警告：账户配置文件已被删除或无法读取，账户信息已清空。");
								}
							}
						}
					}
				} catch (Exception e) {
					System.out.println("错误：用户配置文件更改监听失败，该功能已失效，kiftd无法实时更新用户配置（可尝试重启程序以恢复该功能）。");
				}
			});
	...
	public boolean removeAddedAuthByFolderId(List<String> fIds) {
		...
		synchronized (accountp) {
			...
			try (FileOutputStream accountSettingOut = new FileOutputStream(this.confdir + ACCOUNT_PROPERTIES_FILE)) {
				...
				System.out.println("错误：更新账户配置文件时出现错误，请立即检查账户配置文件。");
				return false;
			}
		}
	}
